...
test.describe('Notifications — canal in-app réel', () => {
  test('API — liste, compteur cohérent avec la base, marquage lu décrémente', async ({ request }) => {
    const token = await apiLogin(request, USERS.p1.email)
    ...
    expect(notifs.length, 'P1 a reçu des notifications lors des transitions de la recette').toBeGreaterThan(0)
    ...
    // Isolation : un autre utilisateur ne peut pas marquer les notifications de P1
    if (notifs.length > 0) {
      const tokenAutre = await apiLogin(request, USERS.p1b.email)
